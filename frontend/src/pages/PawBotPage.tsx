  ...
  return (
    <div className="grid gap-6 lg:grid-cols-[0.8fr_1.2fr]">
      <section className="panel p-8">
        <h1 className="text-3xl font-black">PawBot</h1>
        <p className="mt-3 text-sm text-ink/70">Ask quick pet-care questions. The bot will answer calmly and tell you when a real vet visit is urgent.</p>
        <div className="mt-6">
      ...
      <section className="panel flex min-h-[32rem] flex-col p-6">
        <div className="flex-1 space-y-4 overflow-y-auto rounded-[24px] bg-sand/50 p-4">
          {history.length ? history.map((item, index) => <ChatBubble key={index} role={item.role} text={item.text} />) : <p className="text-sm text-ink/60">Start with a symptom, diet, wound, vaccine, or emergency question.</p>}
